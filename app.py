...
if role == "Employee":
    ...
    if not st.session_state.manager_logged_in:
                ...
                if not pending_df.empty:
                    ...
                    if col_p1.button("💾 Save Edits Only", key="btn_save_edits"):
                        ...
                        for _, row in edited_pending.iterrows():
                            ...
                            if cell:
                                row_num = cell.row
                                # Always recalculate total_hours from the (possibly edited) clock_in/clock_out,
                                # so manually correcting a time actually updates the hours.
                                try:
                                    t_in = datetime.strptime(str(row['clock_in']), "%Y-%m-%d %H:%M:%S")
                                    t_out = datetime.strptime(str(row['clock_out']), "%Y-%m-%d %H:%M:%S")
                                    recalculated_hours = round((t_out - t_in).total_seconds() / 3600.0, 2)
                                except Exception:
        ...
        with mgr_tab5:
            ...
            st.divider()
            st.markdown("#### 🛠️ Fix a Punch Record")
            st.caption("Use this to correct any shift record — including ones already approved. Total hours recalculate automatically from the times you enter.")

            all_punches_settings = get_as_df("time_punches")
            all_emps_settings = get_as_df("employees")

            if all_punches_settings.empty:
                st.info("No punch records found yet.")
            else:
                emp_name_map_settings = {}
                if not all_emps_settings.empty and 'emp_id' in all_emps_settings.columns and 'name' in all_emps_settings.columns:
                    emp_name_map_settings = dict(zip(all_emps_settings['emp_id'].astype(str), all_emps_settings['name']))

                def label_punch(row):
                    name = emp_name_map_settings.get(str(row.get('emp_id', '')), row.get('emp_id', 'Unknown'))
                    return f"{row.get('punch_id')} — {name} — {row.get('clock_in')} ({row.get('shift_name', '')})"

                punch_options = {label_punch(row): row['punch_id'] for _, row in all_punches_settings.iterrows()}
                selected_label = st.selectbox("Select Shift Record", list(punch_options.keys()), key="fix_punch_select")
                selected_punch_id = punch_options[selected_label]
                punch_row = all_punches_settings[all_punches_settings['punch_id'] == selected_punch_id].iloc[0]

                with st.form("fix_punch_form"):
                    try:
                        default_in_date = datetime.strptime(str(punch_row['clock_in']), "%Y-%m-%d %H:%M:%S").date()
                        default_in_time = datetime.strptime(str(punch_row['clock_in']), "%Y-%m-%d %H:%M:%S").time()
                    except Exception:
                        default_in_date, default_in_time = nepal_today(), time(6, 0)

                    try:
                        default_out_date = datetime.strptime(str(punch_row['clock_out']), "%Y-%m-%d %H:%M:%S").date()
                        default_out_time = datetime.strptime(str(punch_row['clock_out']), "%Y-%m-%d %H:%M:%S").time()
                    except Exception:
                        default_out_date, default_out_time = nepal_today(), time(14, 0)

                    fp_c1, fp_c2 = st.columns(2)
                    with fp_c1:
                        fp_in_date = st.date_input("Clock In Date", value=default_in_date, key="fp_in_date")
                        fp_in_time = st.time_input("Clock In Time", value=default_in_time, key="fp_in_time")
                    with fp_c2:
                        fp_out_date = st.date_input("Clock Out Date", value=default_out_date, key="fp_out_date")
                        fp_out_time = st.time_input("Clock Out Time", value=default_out_time, key="fp_out_time")

                    fp_c3, fp_c4, fp_c5 = st.columns(3)
                    with fp_c3:
                        fp_cash_in = st.number_input("Cash In", min_value=0.0, value=safe_float(punch_row.get('cash_in', 0)), step=1.0, key="fp_cash_in")
                    with fp_c4:
                        fp_cash_out = st.number_input("Cash Out", min_value=0.0, value=safe_float(punch_row.get('cash_out', 0)), step=1.0, key="fp_cash_out")
                    with fp_c5:
                        fp_bonus = st.number_input("Bonus", min_value=0.0, value=safe_float(punch_row.get('bonus', 0)), step=1.0, key="fp_bonus")

                    fp_submit = st.form_submit_button("💾 Save Corrected Record", type="primary")

                    if fp_submit:
                        fp_dt_in = datetime.combine(fp_in_date, fp_in_time)
                        fp_dt_out = datetime.combine(fp_out_date, fp_out_time)
                        if fp_dt_out <= fp_dt_in:
                            st.error("Clock Out must be after Clock In. Double-check the dates if this shift crossed midnight.")
                        else:
                            fp_hours = round((fp_dt_out - fp_dt_in).total_seconds() / 3600.0, 2)
                            execute_query(
                                "time_punches", "update", row_id=selected_punch_id,
                                update_dict={
                                    "clock_in": fp_dt_in.strftime("%Y-%m-%d %H:%M:%S"),
                                    "clock_out": fp_dt_out.strftime("%Y-%m-%d %H:%M:%S"),
                                    "total_hours": fp_hours,
                                    "cash_in": float(fp_cash_in),
                                    "cash_out": float(fp_cash_out),
                                    "bonus": float(fp_bonus)
                                }
                            )
                            st.success(f"Record updated: {fp_hours} hours recalculated.")
                            st.rerun()
